...
if __name__ == "__main__":
    with torch.autograd.set_detect_anomaly(True):
        ...
        if "-pretrain_task" in opts:
            ...
            print("pretrain task is "+str(pretrain_task))
            if pretrain_task == "enc_NTP":
                pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/enc_NTP/states_"+str(thiscount)+".pt"
            elif pretrain_task == "genre_NTP":
                pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/genre_NTP/states_"+str(thiscount)+".pt"
            elif pretrain_task == "enc_bachSS":
                pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/enc_bachSS/states_"+str(thiscount)+".pt"
            elif pretrain_task == "enc_shanxiSS":
                pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/enc_shanxiSS/states_"+str(thiscount)+".pt"
        ...
        if orig_dataset=="genre_NTP":
            ...
            with open("/isi/music/auditoryimagery2/seanthesis/enculturation/datasets/enc/X_trainNTP.p", "rb") as samples_fp:
                train_X = pickle.load(samples_fp)
            with open("/isi/music/auditoryimagery2/seanthesis/enculturation/datasets/enc/y_trainNTP.p", "rb") as labels_fp:
                train_Y = pickle.load(labels_fp)
            with open("/isi/music/auditoryimagery2/seanthesis/enculturation/datasets/enc/X_valNTP.p","rb") as samples_fp:
                val_X = pickle.load(samples_fp)
            with open("/isi/music/auditoryimagery2/seanthesis/enculturation/datasets/enc/y_valNTP.p","rb") as labels_fp:
                val_Y = pickle.load(labels_fp)
        ...
        for e in range(1, hp_dict["EPOCHS"]+1):
            ...
            if pretrain_task in ["enc_bachSS", "enc_shanxiSS"]:
                model.eval()
            epoch_loss = 0
            epoch_acc = 0
            epoch_acc2 = 0
            ...
            for X_batch, y_batch in train_loader:
                ...
                if pretrain_task in ["enc_bachSS", "enc_shanxiSS"]:
                    ...
                    batch_mask_indices = "finetune_switch"
                #print("batch mask indices is "+str(batch_mask_indices))
                ypred_bin_batch = model(X_batch, batch_mask_indices)
                ...
                if pretrain_task not in ["enc_bachSS", "enc_shanxiSS"]:
                    loss.backward()
                    optimizer.step()
                batch_count+=1
                ...
                with torch.no_grad():
                    ...
                    for X_batch_val, y_batch_val in val_loader:
                        ...
                        if pretrain_task in ["enc_bachSS", "enc_shanxiSS"]:
                            ...
                            batch_mask_indices = "finetune_switch"
                        ypred_bin_batch_val = model(X_batch_val, batch_mask_indices)
        ...
        #         print(name, param.data)
            if save_model:
                if avg_val_acc > best_avg_val_acc: # save model is set by command line argument
                    ...
                    best_val_epoch = e
                    model_path = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/"+str(orig_dataset)+"/states_"+str(thiscount)+".pt"


                    torch.save(model.state_dict(),model_path)
                    model_path = "/isi/music/auditoryimagery2/seanthesis/enculturation/trained_models/"+str(orig_dataset)+"/full_"+str(thiscount)+".pt"
